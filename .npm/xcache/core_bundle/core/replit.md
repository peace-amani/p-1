...
*   **Wolf AI Assistant (JARVIS-style)**: AI-powered prefixless assistant (`lib/wolfai.js`). Toggle on/off via `.wolf on/off` command (`commands/ai/wolf.js`). Config stored in `data/wolfai/wolf_config.json`. Defaults to ON. Triggered by "wolf", "hey wolf", "yo wolf", etc. Uses real AI (GPT-5 → Copilot → Claude → Blackbox → Grok fallback chain) with a custom system prompt to understand natural language and intelligently route to 30+ bot commands. Maintains per-user conversation memory (30 messages, 2hr session, stored in `data/wolfai/conversations/`). Shows typing indicator while thinking. AI decides when to execute a command vs just chat using `[EXECUTE:command:args]` tags in responses. Quick-match fast path for simple commands (menu, ping, alive, etc). Identity enforced as W.O.L.F by WolfTech. Integrated into `index.js` before prefix parsing — checked via `isWolfEnabled()` gate. Respects owner/sudo permissions. Commands: `.wolf on/off/status/clear`.
